...
| `compliance_v2_audit` | Daily 02:15 CT | Compliance V2 nightly catalog audit + auto-apply — evaluates every live variant's HS code/COO readiness (evidence-verified only, fail-closed) and automatically writes any evidence-verified READY plan to Shopify (no human approval step; each write re-checks live Shopify state immediately beforehand and skips anything gone stale). Alerts with READY/REVIEW_REQUIRED counts and applied/skipped write counts |
